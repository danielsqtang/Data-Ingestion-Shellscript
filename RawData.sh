...
sqoop import --connect jdbc:mysql://52.207.2.108/northwind --username temp --password temp --target-dir /user/daniel/projectscript/NorthwindOrders --m 1 \
--query "select OrderID, CustomerID, EmployeeID, OrderDate, RequiredDate, ShippedDate, ShipVia, Freight, ShipName, Replace(ShipAddress, ',', ''), ShipCity, ShipRegion, ShipPostalCode, ShipCountry from Orders where \$CONDITIONS"
